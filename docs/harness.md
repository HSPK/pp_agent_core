...
This document is a design specification, not shipped Python behaviour. The durable multi-lane `AgentHarness` runtime described here is not implemented in upstream TypeScript or in this Python port; Python `AgentHarness` exposes the surface types and configuration accessors, while operations that require the durable run/compaction/navigation engine raise `HarnessNotImplemented`. Treat the transaction/state-machine fragments below as design notation, not callable Python examples.

Where the Python port has existing modules, paths and symbol names below use the Python package layout (`packages/pi-agent/src/pi_agent/...`) and snake_case field names for in-process Python objects. Names that cross a wire or disk boundary, such as JSONL keys, span names, event `type` values, and telemetry attributes, intentionally keep their original spelling.
...
- `AgentMessage`, `AgentTool`, `AgentToolResult`, `QueueMode`, and `ThinkingLevel`: `packages/pi-agent/src/pi_agent/types.py`.
- `AgentEventSink`: `packages/pi-agent/src/pi_agent/agent_loop.py`.
- `Skill`, `PromptTemplate`, `AgentHarnessResources` (`Resources` below), `AgentHarnessTool`, `AgentHarnessStreamOptions`, and `AgentHarnessStreamOptionsPatch`: `packages/pi-agent/src/pi_agent/harness/types.py`.
- `Model`, `Usage`, `StopReason`, `AssistantMessage`, `ImageContent`, provider messages, stream options, and deferred handles: `packages/pi-ai/src/pi_ai/types.py`; `Models`: `packages/pi-ai/src/pi_ai/registry.py`; `RetryPolicy`: `packages/pi-ai/src/pi_ai/utils/retry.py`.
- `CompactionSettings`, `CompactionPreparation`, `CompactResult`, `BranchPreparation`, and `BranchSummaryResult`: `packages/pi-agent/src/pi_agent/harness/compaction/`. Existing preparation and split-turn algorithms remain the implementation starting point unless this document explicitly changes them.
- `TelemetryContext` and typed schema helpers: `packages/pi-telemetry`; the agent-owned schemas remain in `packages/pi-agent/src/pi_agent/harness/telemetry.py`.
- `JSON schema` for durable custom-message registration: `JSON Schema`.
...
Python status: the current port ships `InMemorySessionRepo`/`InMemorySessionStorage` and `JsonlSessionRepo`/`JsonlSessionStorage`; the SQLite backend described here is not ported.

The design describes three encodings of one model — Memory, JSONL, SQLite — and all three pass the same conformance suite (Part 9). Each backend records the session's `storageVersion` (Part 7): a JSONL header field, a SQLite catalog column. Memory sessions are always current. A possible fourth backend — partitioned Postgres — is sketched informatively in Part 6; nothing here depends on it.
...
interface SessionCodecOptions {
  ...
  customMessageSchemas?: Record<string, JSON schema>;  // keyed by custom `role`
}
...
Python status: this subsection is a design for unbuilt pull-based search. The shipped Python search API is the simple scanner documented in `docs/search.md`.
...
Expected errors use the existing `TaggedError` implementation in `harness/result.py`:
...
Python status: `pi_agent.harness.agent_harness.AgentHarness` ports this declared surface only partially. Its accessors exist; `create`, lane operations, durable driving, and recovery raise `HarnessNotImplemented` where the upstream runtime is also unbuilt.
...
Python status: the current `agent_loop.py` is behavior-compatible with the ported loop surface, but the phase-split exports described here (`streamAssistant`, `prepareToolCall`, `executeToolBatch`, and related functions) are design work and are not exported.

The existing `agent_loop.py` remains behavior-compatible and is refactored into these exported phases. Existing fields on `AgentTool`, `AgentToolResult`, and provider messages are retained. Add recovery declaration `replay?: "never" | "safe"` to `AgentTool`; omission means `"never"`. `AgentHarnessTool` inherits it. The `AgentEventSink` below is the existing agent-loop sink, not the harness event listener; the harness adapts agent events into §5.5 events.
...
Python status: this is the upstream implementation plan, not the current Python package state. The port has already implemented some session, JSONL, telemetry, compaction, and agent-loop pieces independently, while the durable harness runtime slices remain unbuilt.
...
- `packages/pi-agent/src/pi_agent/harness/**` and all of its tests are **deletable outright** in slice 1 — no obligation to adapt anything. Salvaging pieces (the compaction preparation/split-turn algorithms for R8–R9, session/codec fragments) is optional and never required.
- `packages/pi-agent/src/pi_agent/agent_loop.py`: preserve behavior; R4 extracts its phases.
- `packages/session-backends/sqlite-node`: S2 may keep the working transaction and lease primitives or start clean.
- Telemetry contracts (`packages/pi-telemetry`, the agent-owned schemas) remain authoritative.
