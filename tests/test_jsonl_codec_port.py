"""Python port of `packages/agent/test/harness/session/jsonl-codec.test.ts`.

Named ``test_jsonl_codec_port`` because ``test_session_jsonl_codec.py``
already holds this port's own codec suite.

TypeScript's ``parseHeader``/``parseMutation`` return a
``{ ok, value } | { ok: false, error }`` result; the Python port raises
``JsonlDecodeError`` instead (the repo's stated error convention), so the
"returns an error" cases assert on the raised exception's ``kind``.
"""
...
from pi_agent.harness.session.types import CustomEntry, OperationStartedRecord, RunIntent


def expect_header_round_trip(header: JsonlV4Header) -> None:
    encoded = encode_header(header)
    assert encoded.endswith("\n")
    assert parse_header(encoded.rstrip()) == header


def expect_mutation_round_trip(mutation: SessionMutation) -> None:
    encoded = encode_mutation(mutation)
    assert encoded.endswith("\n")
    assert parse_mutation(encoded.rstrip()) == mutation


def test_round_trips_every_header_field_with_a_resolved_parent() -> None:
...
def test_round_trips_an_unresolved_legacy_parent_path() -> None:
    expect_header_round_trip(
        JsonlV4Header(
            id="legacy-child",
...
def test_projects_header_and_filesystem_fields_into_metadata() -> None:
...
@pytest.mark.parametrize(
    ("line", "kind"),
    [("{", "syntax"), (json.dumps({"kind": "unknown", "seq": 1}), "schema")],
)
def test_returns_syntax_and_schema_errors(line: str, kind: str) -> None:
    with pytest.raises(JsonlDecodeError) as caught:
        parse_mutation(line)
    assert caught.value.kind == kind


def test_round_trips_a_lane_bound_entry_line() -> None:
...
def test_round_trips_an_imported_entry_line_without_a_lane() -> None:
    expect_mutation_round_trip(
        EntryMutation(entry=CustomEntry(id="entry-1", seq=1, parent_id=None, timestamp=100, custom_type="note"))
    )


def test_round_trips_a_record_line() -> None:
    expect_mutation_round_trip(
        RecordMutation(
...
def test_round_trips_a_lane_line() -> None:
    expect_mutation_round_trip(LaneMutation(seq=1, lane="thread", leaf_id="entry-1"))


def test_round_trips_fact_lines_including_cleared_values() -> None:
...
def test_rejects_invalid_mutation_lines(name: str, mutation: dict[str, Any]) -> None:
    with pytest.raises(JsonlDecodeError):
        parse_mutation(json.dumps(mutation))
